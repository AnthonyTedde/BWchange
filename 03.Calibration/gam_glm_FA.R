...
(mvr_FA_pred <- pls::mvr(formulae_pls, ncomp = 6, data = train_data_noseenorest,
                        method = "oscorespls",
                        scale = T, center = T)) |> system.time()

tibble::tibble(
  pred = predict(mvr_FA_pred, newdata = train_data_noseenorest, comps = 6) |> drop(),
...
formulae_gam <- glue::glue("s({FAs_vip}, k=100, bs = 'tp')") %>%
  ...
  paste("s(fat_rate_2_prd, k=100, bs = 'tp')",
        ...
        "s(lactose_rate_1_prd, k=100, bs = 'tp')",
         sep = "+") %>%
        # "s(dry_matter_intake_prd, bs = 'cr')", sep = "+") %>%
