plot_desc <- function(dat){
  ...
  BW_plt <- dat %>%
    ...
    ggplot2::ggplot(ggplot2::aes(x = bodyweight, y = breed,
                                 color = breed)) +
  ...
  MY_plt <- dat %>%
    ...
    ggplot2::ggplot(ggplot2::aes(x = milk_yield, y = breed,
                                 color = breed)) +
    ggplot2::geom_boxplot() +
    ggsci::scale_fill_nejm(name = "Datasets") +
    ggsci::scale_color_nejm(name = "Datasets") +
  ...
  PRT_plt <- dat %>%
    ...
    ggplot2::ggplot(ggplot2::aes(x = breed,
                                 color = parity,
                                 fill = parity)) +
  ...
  WOL_plt <- dat %>%
    ...
    ggplot2::ggplot(ggplot2::aes(x = as.numeric(wol_fct), y = breed,
                                 color = breed)) +
    ggplot2::geom_boxplot() +
  ...
  spectral_data <- dat %>%
    dplyr::select(breed, an_id, country, dplyr::starts_with("dpin"))
  ...
  pca_prdt <- preprocess_lst %>%
    purrr::imap(function(mdl, dt_breed){
      dt <- spectral_data %>%
        dplyr::filter(breed == dt_breed)
  ...
  spectra_plt <- data_pca %>%
    ...
    ggplot2::ggplot(ggplot2::aes(x = PC1, y = PC2, color = breed)) +
    ggplot2::geom_point() +
    ggsci::scale_fill_nejm() +
    ggsci::scale_color_nejm() +
