...
# swiss <- tibble::tibble(
#   pred = predict(pls_lst$HSO_all, newdata = test_data %>%
#                    dplyr::mutate(parity_4 = ifelse(parity_full > 4, 4, parity_full))),
#   obs = test_data$bodyweight
# )

out_of_sample <- BW_HM_seenorest_2021
# MOdels ####
...
pls_lm_mod_butch <- pls_lst$HSO_all
# pls_lm_mod_butch <- pls_lst$HSO_vip
# pls_lm_mod_butch <- pls_lst$HSO_SBF_beta

out_of_sample %<>%
  dplyr::mutate(
    log_dim = log(dim, exp(1)),
    log_bw = log(bodyweight, exp(1)),
    parity_4 = ifelse(parity > 4, 4, parity)
  )
...
BW_dp <- BW_seenorest %>%
  ...
  dplyr::count(parity_4, expl_id, an_id, parity_4, name = "#BW")

MIR_dp <- out_of_sample %>%
  dplyr::filter(dim < 365) %>%
  dplyr::count(parity_4, expl_id, an_id, parity_4, name = "#MIR")

merged_dp <- dplyr::inner_join(BW_dp, MIR_dp) %>%
  dplyr::ungroup()

merged_dp$`#MIR` %>%
  with(merged_dp, table(`#MIR`, parity))
  table
...
keep_dp <- merged_dp %>%
  ...
  dplyr::filter(parity_4 < 4) %>%
  dplyr::group_by(parity_4) %>%
...
seenorest_mir <- out_of_sample %>%
  dplyr::filter(dim < 365) %>%
  dplyr::inner_join(keep_dp %>% dplyr::select(expl_id, an_id, parity_4),
                    by = c("expl_id", "an_id", "parity_4"))
...
seenorest_prd_mdl <- seenorest_mir %>%
  dplyr::mutate(
    ...
    bodyweight = predict(pls_lm_mod_butch, seenorest_mir),
    log_bw = log(bodyweight, exp(1)),
    log_dim = log(dim, exp(1)),
  ) %>%
  dplyr::group_by(an_id, expl_id, parity_4) %>%
...
# print some points
# an_id == ai & expl_id == ei & parity_4 == 1
# ai <- "5502419996"
ai <- "5502256647"
ei <- "55331006"
pt <- 2
...
k <- seenorest_prd_mdl %>%
  dplyr::filter(an_id == ai & parity_4 == pt) %>%
  dplyr::pull(k)

data_pred <- tibble::tibble(dim = 1:365, wil = exp(dim*k))
dat_prt <- seenorest_prd_mdl %>%
  dplyr::filter(an_id == ai & expl_id == ei & parity_4 == pt)
...
# print some points
# an_id == "5502092446" & expl_id == ei & parity_4 == 3
...
#   ) %>%
#   dplyr::select(an_id, expl_id, parity_4, max_dim, min_dim,
